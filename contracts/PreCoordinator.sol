...
import "./HashBridgeOracleTokenReceiver.sol";
import "./Median.sol";
import "./vendor/Ownable.sol";
import "./vendor/SafeMathHashBridgeOracle.sol";
    ...
    HashBridgeOracleTokenReceiver
{
    ...
    /**
     * @notice Deploy the contract with a specified address for the HBO
     * and Oracle contract addresses
     * @dev Sets the storage for the specified addresses
     * @param _hbo The address of the HBO token contract
     */
    constructor(address _hbo) public {
        if (_hbo == address(0)) {
            setPublicHashBridgeOracleToken();
        } else {
    ...
    /**
     * @notice Returns the address of the HBO token
     * @dev This is the public implementation for hashBridgeOracleTokenAddress, which is
     * an internal method of the HashBridgeOracleClient contract
     */
    function getHashBridgeOracleToken() public view override returns (address) {
        return hashBridgeOracleTokenAddress();
    }

    /**
     * @notice Creates the HashBridgeOracle request
     * @dev Stores the hash of the params as the on-chain commitment for the request.
     * Emits OracleRequest event for the HashBridgeOracle node to detect.
    ...
    function oracleRequest(
    ...
    ) external override onlyHBO checkCallbackAddress(_callbackAddress) {
        ...
        bytes32 callbackRequestId =
            keccak256(abi.encodePacked(_sender, _nonce));
        require(
        ...
        if (_payment > totalPayment) {
            ...
            HashBridgeOracleTokenInterface _hbo =
                HashBridgeOracleTokenInterface(hashBridgeOracleTokenAddress());
            assert(_hbo.transfer(_sender, overage));
        }
    }

    /**
     * @dev Creates HashBridgeOracle requests to each oracle in the service agreement with the
    ...
    function createRequests(
        ...
        HashBridgeOracle.Request memory request;
        ...
        for (uint256 i = 0; i < sa.oracles.length; i++) {
            request = buildHashBridgeOracleRequest(
                sa.jobIds[i],
                address(this),
                this.hashBridgeOracleCallback.selector
            );
            request.setBuffer(_data);
            outgoingRequestId = sendHashBridgeOracleRequestTo(
                sa.oracles[i],
                request,
                sa.payments[i]
            );
     ...
     * @dev The recordHashBridgeOracleFulfillment protects this function from being called
     * by anyone other than the oracle address that the request was sent to
     * @param _requestId The ID that was generated for the request
     * @param _data The answer provided by the oracle
     */
    function hashBridgeOracleCallback(bytes32 _requestId, int256 _data)
        external
        recordHashBridgeOracleFulfillment(_requestId)
        returns (bool)
    {
        ServiceAgreement memory sa =
            serviceAgreements[serviceAgreementRequests[_requestId]];
    ...
    /**
     * @notice Allows the owner to withdraw any HBO balance on the contract
     * @dev The only valid case for there to be remaining HBO on this contract
     * is if a user accidentally sent HBO directly to this contract's address.
     */
    function withdrawHbo() external onlyOwner {
        HashBridgeOracleTokenInterface _hbo =
            HashBridgeOracleTokenInterface(hashBridgeOracleTokenAddress());
        require(
            _hbo.transfer(msg.sender, _hbo.balanceOf(address(this))),
            "Unable to transfer"
        );
    }
    ...
    function cancelOracleRequest(
        ...
        HashBridgeOracleTokenInterface _hbo =
            HashBridgeOracleTokenInterface(hashBridgeOracleTokenAddress());
        require(_hbo.transfer(req.sender, _payment), "Unable to transfer");
    }

    /**
     * @dev Reverts if the callback address is the HBO token
     * @param _to The callback address
     */
    modifier checkCallbackAddress(address _to) {
        require(
            _to != hashBridgeOracleTokenAddress(),
            "Cannot callback to HBO"
        );
        _;
    }
}
